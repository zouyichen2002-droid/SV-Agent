...
| | |
|---|---|
| 更新 | 2026-09-05，直写 `.flp`（ADR-0013）之后 —— **配器不再逐条手挂** |
...
**你只做三件事**：给一句主题、选一版歌词、在 FL 里按导出。其余我做。

（配器以前也在你这边，现在不用了 —— 见下面「你在 FL 里两步」。）
...
**前提：`project.json` 的 `flp` 指向一份已经挂好音源的工程。**
有它的话我直接拼出一份带配器的工程给你：

```bash
python E:/sv-bridge/scripts/sv.py act build_flp
```

产物是 `<歌名>_伴奏.flp`，**不会覆盖你自己那份工程**。然后你只做两步：

1. 打开 `<歌名>_伴奏.flp` —— **音源已经挂好了，五条通道都在**
2. 导出到 `<歌名>_伴奏.wav`

不满意配器就让我换几版，音符一个字不动：

```bash
python E:/sv-bridge/scripts/sv.py act reorchestrate --params '{"remap":{"0":3,"3":0}}'
```

**为什么导出还得你按**：FL 有没有命令行渲染，至今没确认（见 ADR-0013 的缺口表）。

### 第一首歌，或者要新音色时

模板得先有。**这一次挂音源仍然要你手工做**：导入 MIDI、按通道拆分、
逐条挂音源，存成工程，把路径写进 `project.json` 的 `flp`。

之后每首歌都复用它，不用再挂。想加第六个音源也是同样 ——
**挂一次，不是每首歌一次**。
...
| 文件 | 内容 |
|---|---|
| `specs/facts.md` | **18 条环境硬约束**，每条都写了「不知道会怎样」和「怎么学到的」 |
