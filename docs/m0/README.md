...
| **5** | 文件并发编辑 | **绿** · SynthV 和 FL 都不察觉外面的改动、一保存就静悄悄覆盖 → 一律写新文件；FL 开着时命令行导出会失败 → 导出前先确认 FL 没开 | [05-concurrency.md](05-concurrency.md) |
| **6** | 所谓 512 音符限制 | **绿** · 是上游桥 0.3.1 自己的每次调用上限，不是 SynthV 的；直写 `.svp` 碰不到 | [06-note-limit.md](06-note-limit.md) |
| 7 | 首批平台发布入口 | 未开始 · **要你本人账号操作** | —— |

**退出条件两半都满足了**：预览有可行路径；FL、SynthV、并发编辑的风险都有了替代方案。只剩第 7 项（要你的账号）。
