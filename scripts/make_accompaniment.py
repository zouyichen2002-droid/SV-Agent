...
# → 2026-09-05 改成 pad 49–61 / arp 74–85，理由见下面 BANDS 的注释。
#   当初这套「让开 56–76」是对的取舍，代价是伴奏单独听很空。
# 副线在 64–71，那是旋律的地盘，所以它**只在人声缺席的段落**出现。
...
# 2026-09-05 重排。实测《风筝线》的分布是：
#
#     bass 34–48  ┐ **重叠 7 个半音** → 低频糊成一团
#     pad  41–51  ┘
#          52–66    ← 空
#     counter 67–70
#     arp  81–87
#
# **只有第一处是缺陷。** 手册「低频区两个同时发声的音至少隔一个纯五度」，
# 两个持续声部挤在 41–48 就是糊的来源。
#
# 中间那个洞**是故意的**（见上面那段旧注释）：垫顶 ≤55、琶音 ≥77，
# 把 56–76 整段让给人声。这条不是 bug，是取舍。
#
# 但代价是**伴奏单独听很空**，创作者的原话是「太阴间」。
# 于是这里改成折中，而不是把洞填满：
#
#     bass 36–48 │ pad 49–61 │ (62–73 留给人声) │ arp 74–85
#     counter 64–71 —— 只在人声缺席的段落说话（DENSITY 的 cnt_on）
#
#   · pad 从 49 起步：与 bass 彻底分开，糊的根源没了
#   · pad 顶到 61：只碰人声下缘（星尘 57–78，本曲旋律 60–75），不进中枢
#   · arp 从 74 起步（原 77）：多填一点高中频，但仍在旋律主体之上
#
# **未验证的部分**：创作者迄今只听过纯伴奏。这套分带在「伴奏 + 人声」
# 一起听时会不会挤到人声，要等混音之后量人声的中频占比才知道。
BANDS = {"bass": (36, 48), "pad": (49, 61), "arp": (74, 85),
         "counter": (64, 71)}
...
#
# 2026-09-05 修三处硬伤，都是《风筝线》实测出来的：
#
# 一、`hat-4`/`hat-8` 原来底鼓拍是空的 —— 主歌 18 个小节**只有一条镲**。
#    手册的检验法是「跟着打拍子，打不下去的地方就是烂的」，光一条镲
#    你打不出第 1 拍在哪。现在最少也给 1、3 拍两下底鼓。
#
# 二、`ride` 原来每小节都撞一下（实测 16 次，且时值 3.8 拍几乎占满整格）。
#    镲是**段落起点的重音**，不是每小节的常驻件。改成只在段首给一下。
#
# 三、十六分档 `hat-16`。参考曲《借月为心》129 BPM 是连续八分，
#    而 129 ≈ 66 × 2 —— 同样的密度在慢速歌上就是十六分。
#    这样不动速度也能拿到那种推进感。
DRUM_STYLES = {
    "hat-4":    ((0.0, 2.0), (), 1.0, False),
    "hat-8":    ((0.0, 2.0), (), 0.5, False),
    "kick-2":   ((0.0, 2.0), (), 0.5, False),
    "backbeat": ((0.0, 2.0), (1.0, 3.0), 0.5, False),
    "hat-16":   ((0.0, 2.0), (1.0, 3.0), 0.25, True),
...
# 加花：乐句末（4 小节一次）的第 4 拍后三个十六分。只用拍手 ——
# 通鼓的音高要赌 FPC 挂的是哪套鼓组，赌输的表现是「某一下没声音」，不报错。
#
# **从 3.25 起，不是 3.0**：backbeat 的拍手本来就在 3.0，
# 从 3.0 起会在同一位置同一音高写两个音符 —— step4 的回读复验当场报
# 「Drums 不一致」，听感上则是重影。同理不放开镲：
# 十六分镲的收尾本来就在 3.75 放了一个。
FILL = [(3.25, 0.15, CLAP, 70), (3.5, 0.15, CLAP, 78),
        (3.75, 0.15, CLAP, 86)]
...
def build_parts(mod):
    ...
    parts: dict[str, list] = {k: [] for k in
                              ("垫", "贝斯", "琶音", "副线", "鼓")}
    # 每个小节所属段落的**起始小节号**。叮叮与加花都按段内位置走，
    # 先算出来 —— 在循环里往回找是 O(n²)，而且差一格不会报错，只会听着怪。
    #
    # **`sec` 是 dict 不是 list。** 第一版写成 `enumerate(sec)`，
    # 拿到的是键（小节号）不是段名，于是 `sec[b-1] != nm` 变成
    # 「字符串 != 整数」永远成立 —— 每个小节都成了段落起点，
    # 结果叮叮照旧每小节响、加花一次都轮不到。**241 个测试全绿。**
    # 是把输出的数字拉出来对才发现的，不是测试发现的。
    sec_start: list[int] = []
    for b in range(mod.N_BARS):
        nm = sec[b]
        sec_start.append(b if (b == 0 or sec[b - 1] != nm) else sec_start[-1])
    ...
    for b in range(mod.N_BARS):
        ...
        if hat_div:
            ...
            for i in range(n_hat):
                ...
                # 十六分要分三级给力度。**全一样响就是机器打的** ——
                # 参考曲实测正拍 15%、反拍 10%，有重音但不悬殊。
                if hat_div <= 0.25:
                    vel = 62 if i % 4 == 0 else (52 if i % 2 == 0 else 44)
                else:
                    vel = 48 + (i % 2) * 10
                parts["鼓"].append((t0 + i * hat_div,
                                   min(0.2, hat_div * 0.9),
                                   OPENHAT if last else HAT,
                                   vel + (8 if last else 0)))
        # 叮叮**只在段落起点**。每小节都撞一下（而且长 3.8 拍）是
        # 《风筝线》那次最刺耳的一处，实测 16 次 → 现在 4 次。
        if ride and b == sec_start[b]:
            parts["鼓"].append((t0, 1.0, RIDE, 52))
        # 加花：段内每 4 小节的最后一格。手册「3–4 小节一次，用来推情绪」，
        # 而且**包括段落最后一小节** —— 那一下正是推进下一段的。
        if drum != "none" and (b - sec_start[b]) % 4 == 3:
            for off, dur, key, vel in FILL:
                parts["鼓"].append((t0 + off, dur, key, vel))
    return parts, ch, sec
