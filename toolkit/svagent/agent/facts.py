...
FACTS: list[Fact] = [
    ...
    Fact("F16", "项目约定",
         "**创作者会随时手改**：FL 工程、SynthV 工程、歌词 txt，"
         "三处都可能在任何时候被他动过，而且不预告",
         "任何「我上次写的还在」的假设都会错。哈希校验（建造第 1 项）"
         "就是为这条建的 —— 写之前必须比对内容，对不上就**拒绝覆盖**，"
         "不是提醒、不是备份后照写。\n"
         "    **FL 那一侧我们看不见过程**，只能看到导出的 wav 变了；"
         "所以 FLP 要进 `proj.sources`，让他的编曲工作也能被快照与回滚。",
         "创作者 2026-08-25 端到端验收《风筝线》之后明确交代："
         "「我其实做了一些修改，这次主要是在 FL 中……"
         "后面可能会改 SV 和 txt，而且会随时改」",
         None, MANUAL),

    Fact("F17", "混音",
         "**人声要压得住伴奏。** 伴奏默认增益从 −3 dB 降到 −6 dB",
         "创作者实听《风筝线》的判断：「歌声对比伴奏有点弱」。"
         "挖槽（carve）只解决频段打架，解决不了整体音量关系",
         "创作者 2026-08-25 实听给的。**这类判断只有耳朵能下**，"
         "指标测不出来 —— 所以它是 manual，永远不会有自动复验",
         None, MANUAL),

    Fact("F18", "凭据",
         "Mistral 的 key **与他人共用，不能轮换**。"
         "所以泄露的代价不是「换一个」，是几个人一起受影响",
         "这个仓库是公开的。防线因此从「万一漏了就轮换」变成"
         "「必须拦住」——`.githooks/pre-commit` 调 `scripts/scan_secrets.py`，"
         "扫暂存区，命中就退出码 1。"
         "扫描器**只存 SHA-256 不存明文**，报错也打码 ——"
         "一个保护凭据的工具反手把凭据打到终端上，等于白做。",
         "创作者 2026-08-25 明确：「mistral 不能换，因为这个 key 是我和"
         "几个人共用的」。此前我已失手过一次：`test_llm.py` 的「同格式假 key」"
         "抄了真 key，靠提交前手工 grep 拦下 —— 手工靠的是记性",
         None, MANUAL),
