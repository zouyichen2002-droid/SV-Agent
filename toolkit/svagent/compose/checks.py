...
class CheckCfg:
    ...
    #
    # 2026-09-06 换了依据。原来是拿本项目自己的作品校的，
    # 而创作者明确说过那些歌「都是用来测试接入能力的，不要进经验」。
    # 现在锚在 **582 首真歌的 16326 个行级乐句**上（POP909，见 reference.py）：
    #
    #     p5 0.19　p10 0.28　p25 0.42　中位 0.58
    #
    # 0.30 落在真歌的 p10 附近，会判掉 11.8% 的真歌乐句 —— 略严但站得住。
    # **数值没动，动的是依据**：原来的理由是错的，落点碰巧合理，
    # 为了改而改只会制造无谓的变更。
...
def check_chord_fit(notes: list[Note], phrases: list[Phrase],
    ...
    按个数算会把大量正常的句子判成问题，**那不是严格，那是假阳性**。

    这个选择当初是拿本项目自己的作品验的（按个数 4 句超标、按时长 0 句）。
    那个依据已作废（创作者：那些歌是测试产物，不要进经验），
    但**结论本身与依据无关** —— 「短经过音是装饰」是音乐事实，
    不是某一首歌的性质。真歌数据也支持：582 首的行级乐句按时长加权，
    中位 0.58，若按个数会显著更低。
